...
class FileValidatorTest {
    ...
    fun `test is markdown file`(file: String, expected: Boolean) {
    ...
    fun `test is index file`(file: String, expected: Boolean) {
    ...
    fun `test validate files contains missing index`() {
        ...
        assertFalse(
            FileValidator.containsIndex(listOf(fileMarkdown, fileMissing, fileMissingIndex))
        )
    }

    @ParameterizedTest
    @MethodSource("supportedFiles")
    fun `test validate files supported by libre office`(file: String, expected: Boolean) {
